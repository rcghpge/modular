...
def test_openai_extract_image_from_requests() -> None:
    request_images = {
    ...
    messages, images = openai_parse_chat_completion_request(request, False)
    assert len(messages) == 2
    assert len(images) == 0
    assert "text" in messages[1]["content"][0]

    messages, images = openai_parse_chat_completion_request(request, True)
    assert len(messages) == 2
    assert len(images) == 0
    ...
    messages, images = openai_parse_chat_completion_request(
        request,
        False,
    )
