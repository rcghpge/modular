...
def test_single_lora_scheduling() -> None:
    ...
    assert len(output.batch_inputs) == 1
    assert ctx.request_id in output.batch_inputs
...
def test_multi_lora_within_budget() -> None:
    ...
    assert len(output.batch_inputs) == 3
    assert ctx1.request_id in output.batch_inputs
    assert ctx2.request_id in output.batch_inputs
    assert ctx3.request_id in output.batch_inputs
...
def test_lora_preemption_over_budget() -> None:
    ...
    assert len(output.batch_inputs) == 3
    assert ctx_base.request_id in output.batch_inputs  # Base always scheduled
    # First 2 LoRAs should be scheduled
    assert ctx_lora1.request_id in output.batch_inputs
    assert ctx_lora2.request_id in output.batch_inputs
    # Third LoRA should be deferred
    assert ctx_lora3.request_id not in output.batch_inputs
...
def test_age_based_scheduling_with_lora() -> None:
    ...
    assert len(output.batch_inputs) == 2
    assert ctx_inactive.request_id in output.batch_inputs
    assert ctx_base.request_id in output.batch_inputs
...
def test_tg_batch_with_active_loras() -> None:
    ...
    assert len(output.batch_inputs) == 3
    assert ctx_active1.request_id in output.batch_inputs
    assert ctx_active2.request_id in output.batch_inputs
    assert ctx_base.request_id in output.batch_inputs
...
def test_lora_swapping_ce_to_tg() -> None:
    ...
    assert ctx.request_id in tg_output.batch_inputs
    assert ctx2.request_id in tg_output.batch_inputs
...
def test_mixed_requests_scheduling() -> None:
    ...
    assert len(output.batch_inputs) == 3
    assert ctx_base1.request_id in output.batch_inputs
    assert ctx_base2.request_id in output.batch_inputs
    # One of the LoRA requests should be scheduled
    assert (ctx_lora1.request_id in output.batch_inputs) or (
        ctx_lora2.request_id in output.batch_inputs
    )
