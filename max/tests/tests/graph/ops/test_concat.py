...
import pytest
from conftest import GraphBuilder, axes, shapes, symbolic_axes, tensor_types
...
def test_concat_different_devices(graph_builder: GraphBuilder) -> None:
    input_types = [
    ...
    with (
        ...
        pytest.raises(
            ValueError, match=r"Input values must be on the same device.*"
        ),
    ):
        ops.concat([i.tensor for i in graph.inputs], 0)
