...
@pytest.mark.parametrize(
    ...
    [
        ...
        TensorType(DType.float64, ["dim"], device=DeviceRef.CPU()),
    ],
)
def test_mlp(session: InferenceSession, input_type: TensorType) -> None:
    dim = input_type.shape[-1]
    w1_type = TensorType(
        input_type.dtype, ["hidden_dim", dim], device=DeviceRef.CPU()
    )
    w2_type = TensorType(
        input_type.dtype, [dim, "hidden_dim"], device=DeviceRef.CPU()
    )
