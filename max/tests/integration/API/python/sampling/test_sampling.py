...
def test_batch_sampling_arguments(session: InferenceSession) -> None:
    ...
    def test_top_p_sampling() -> None:
        ...
        min_top_p_tensor = Tensor.from_numpy(
...
def test_rejection_sampler_with_residuals(session: InferenceSession) -> None:
    ...
    first_rejected_token, recovered_tokens, bonus_tokens = sampler(
        ...
        Tensor.from_dlpack(target_logit_offsets).to(device),
        Tensor.from_dlpack(draft_logits_tensor).to(device),
        Tensor.from_dlpack(rejection_rand).to(device),
        Tensor.from_dlpack(
