...
def test_is_nan(session: InferenceSession, dtype) -> None:  # noqa: ANN001
    ...
    with Graph(f"is_nan_{dtype}", input_types=[input_type]) as graph:
        out = ops.is_nan(graph.inputs[0].tensor)
        graph.output(out)
    ...
    torch_result = torch.isnan(input_data).cpu().numpy()
