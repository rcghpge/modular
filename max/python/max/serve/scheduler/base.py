...
class PrefillResponse(
    ...
    Attributes:
        id: Unique identifier for this request
        generated_token_id: The first generated token from prefill
        transfer_metadata: The transfer metadata for the KV cache transfers
        draft_tokens: Draft tokens generated by the Eagle/MTP draft model
            during prefill.  When present, the decode node can feed these
            into its first speculative-decode iteration instead of starting
            with an empty draft cache.
    """
