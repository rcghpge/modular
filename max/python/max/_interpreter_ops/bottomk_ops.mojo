...
"""Mojo kernel wrappers for bottom_k MO interpreter operations.

Bottom-K selects the k smallest values and their original indices along a given
...
Returned values are in ascending order (stable); the `sorted` flag is
...
def PyInit_bottomk_ops() -> PythonObject:
    ...
    try:
        ...
        b.def_function[bottomk_dispatcher](
            "BottomK", docstring="Bottom-K along axis"
        )
        return b.finalize()
    except e:
        abort(t"failed to create bottomk op bindings module: {e}")
...
def bottomk_op[
    ...
    Args:
        ...
        dim0: Product of dimensions before the bottom-k axis.
    ...
    def func[width: Int, rank: Int, alignment: Int = 1](idx: IndexList[rank]):
        ...
        for ki in range(k):
            var best_val = Scalar[dtype].MAX_FINITE
...
@fieldwise_init
struct _BottomKBody(Dispatchable):
    """Dispatch body for the BottomK operation over numeric data dtypes.
    ...
    def call[t: DType](self) raises -> None:
        comptime if t.is_numeric():
            ...
            raise Error("bottom_k requires a numeric dtype, got " + String(t))


def bottomk_dispatcher(
    ...
    """Bottom-K dispatcher: unwraps PythonObjects and dispatches by dtype.
