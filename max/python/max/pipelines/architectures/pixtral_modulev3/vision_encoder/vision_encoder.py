...
@dataclass
class VisionEncoder(Module[..., Tensor]):
    ...
    def forward(self, imgs: list[Tensor], attention_mask: Tensor) -> Tensor:
        """
        imgs: list of images of shape = (num_channels, height, width)
        """
        ...
        patch_embeds_list = [
            F.cast(
                self.patch_conv(F.unsqueeze(F.cast(img, self.dtype), 0)),
                self.dtype,
            )
            for img in imgs
        ]
        ...
        patch_embeds_normed = self.layer_norm(patch_embeds_tv)  # type: ignore[arg-type]
