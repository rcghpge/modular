...
class Qwen2_5VLModel(
    ...
    def prepare_decoder_position_ids(
        ...
        Args:
            ...
            parallel_ops: Parallel array operations instance for concatenation
        ...
        for ctx in context_batch:
            ...
            if (
                ...
                position_ids_list.append(
                    ctx_decoder_position_ids[:, ctx.start_idx : ctx.active_idx]
                )
        ...
        decoder_position_ids = Tensor.from_numpy(
            parallel_ops.concatenate(position_ids_list, axis=1).astype(
                np.uint32
            )
        ).to(devices[0])
    ...
    def prepare_initial_token_inputs(
        ...
        with Tracer("prepare_decoder_position_ids"):
            decoder_position_ids = self.prepare_decoder_position_ids(
                context_batch, self._parallel_ops, self.devices
            )
