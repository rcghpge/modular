...
_SHARED_MEMORY_WATERMARK = float(os.getenv("MODULAR_MAX_SHM_WATERMARK", 0.9))
...
def _can_allocate(size: int) -> bool:
    ...
    return size < available * _SHARED_MEMORY_WATERMARK
...
def _ndarray_to_shared_memory(
    ...
    if not _can_allocate(arr.nbytes):
        global _LAST_WARNING
        if time.monotonic() - _LAST_WARNING > _WARNING_INTERVAL:
            _LAST_WARNING = time.monotonic()
            _logger.warning(
    ...
    try:
        ...
        _logger.warning(f"Failed to create shared memory array: {e}")
        return None


def _open_shm_array(meta: dict[str, Any]) -> npt.NDArray[Any]:
