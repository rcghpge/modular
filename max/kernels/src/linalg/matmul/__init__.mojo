...
def matmul[
    ...
    comptime if not is_cpu[target]():
        ...
        # CPU path: convert to NDBuffer for cpu.matmul.
        comptime dim[i: Int] = Dim(i) if i > -1 else Dim()

        comptime c_shape = DimList[
            dim[c.static_shape[0]], dim[c.static_shape[1]]
        ]()
        ...
        comptime a_shape = DimList[
            dim[a.static_shape[0]], dim[a.static_shape[1]]
        ]()
        ...
        comptime b_shape = DimList[
            dim[b.static_shape[0]], dim[b.static_shape[1]]
        ]()
