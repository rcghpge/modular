        ...
        for j in range(k_group_size):
            ...
            sfa_tma_op.async_copy_5d[cta_group](
                ...
                (
                    ...
                    UInt(0),
                    UInt(iter_idx + j),
                    work_tile_coord[0] * UInt(BM // SF_MN_GROUP_SIZE),
                    UInt(batch_coord),
                ),
            )
            sfb_tma_op.async_copy_5d[cta_group](
                ...
                (
                    ...
                    UInt(0),
                    UInt(iter_idx + j),
                    work_tile_coord[1] * UInt(MMA_N // SF_MN_GROUP_SIZE),
                    UInt(batch_coord),
                ),
            )
...
fn blackwell_block_scaled_tma_umma_warp_specialized_kernel[
    ...
    if WarpRole.is_mma():
        with MatmulProfilerType[2](workspace, 0):
            ...
            var sfa_tmem = tmem_addr + config.num_accum_pipeline_stages * UInt(
                MMA_N
            )
            var sfb_tmem = (
                sfa_tmem + UInt(SFA_NUM_COLS) * config.num_pipeline_stages
            )
