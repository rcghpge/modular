...
struct MLAAttentionConfig[token_gen: Bool, config: MHAConfig](AttentionConfig):
    # Prefill uses gfx950-style: separate K/V SMEM, full tile, double-buffered.
    # Decode uses original gfx942-style: shared K/V SMEM, BK blocks, single-buffered.
    comptime shared_kv = True if Self.token_gen else False
    comptime full_kv = False if Self.token_gen else True
    comptime depth_padded = True if Self.token_gen else False
    comptime double_buffer = False if Self.token_gen else True
    ...
    def mla_prefill_old[
        ...
        def loop_over_kvcache[
            ...
            comptime KRopeTileLayout = MixedLayout[
                ...
                Coord[
                    ComptimeInt[_k_rope_stride0], ComptimeInt[1]
                ].element_types,
            ]
