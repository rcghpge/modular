...
def run_rms_norm_fused_residual_add_gpu[
    ...
    def fused_output_fn[
        ...
        result_fused_buf.flat_store[width=width, alignment=alignment](idx, val)
    ...
    def fused_residual_output_fn[
        ...
        residual_fused_output_buf.flat_store[width=width, alignment=alignment](
            idx, val
        )
    ...
    def unfused_output_fn[
        ...
        unfused_intermediate_buf.flat_store[width=width, alignment=alignment](
            idx, val
        )
    ...
    def sum_fn[
        ...
        var residual_val = data_buf.flat_load[width=width](data_buf_idx)
        var unfused_intermediate_buf_idx = unfused_intermediate_buf.layout(
            Coord(coords)
        )
        var result_val = unfused_intermediate_buf.flat_load[width=width](
            unfused_intermediate_buf_idx
        )

        var residual_add_val = residual_val + result_val
        unfused_intermediate_buf.flat_store[width=width](
    ...
    def unfused_input2_fn[
        ...
        var idx = unfused_intermediate_buf.layout(Coord(coords))
        return unfused_intermediate_buf.flat_load[width=width](idx)
    ...
    def unfused_output2_fn[
        ...
        result_unfused_buf.flat_store[width=width, alignment=alignment](
            idx, val
        )
    ...
    for i in range(flattened_size):
        assert_almost_equal(
            result_fused_h.flat_load(i),
            result_unfused_h.flat_load(i),
            rtol=rtol,
        )
        assert_almost_equal(
            residual_fused_output_h.flat_load(i),
            unfused_intermediate_h.flat_load(i),
            rtol=rtol,
        )
    data_heap.free()
