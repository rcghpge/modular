...
def test_sliced_add[
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            a_host.flat_store(idx, 1.0)
            b_host.flat_store(idx, 2.0)
            c_host.flat_store(idx, 0.0)
    ...
    lora_end_idx_host.flat_store(0, Int64(batch_end_idx))
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            var actual = c_host.flat_load(idx)
            if actual != expected:
                raise Error(
                    "Mismatch at ["
                    + String(i)
