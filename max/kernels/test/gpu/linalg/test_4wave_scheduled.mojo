...
# ===----------------------------------------------------------------------=== #
"""AMD 4-wave-scheduled matmul correctness test (FP8 only).

Drives `amd_4wave_scheduled_matmul`, the schedule-compiler arm
of the inline kernel struct. Same scaffolding as `test_4wave.mojo`
but exercises `run_scheduled` instead of `run`.

Compile-time configuration:
  mojo -D M=128 -D N=4096 -D K=4096 test_4wave_scheduled.mojo
...
def test_4wave_scheduled[
    ...
    """Test 4-wave-scheduled FP8 kernel against vendor BLAS."""
...
def main() raises:
    with DeviceContext() as ctx:
        print("Running AMD 4-wave-scheduled Kernel Tests (FP8)")
        print("  Shape: M=", TEST_M, " N=", TEST_N, " K=", TEST_K, sep="")

        print("  Testing without swizzle...")
        test_4wave_scheduled[TEST_M, TEST_N, TEST_K, enable_swizzle=False](ctx)
        print("  PASSED: No swizzle")

        print("  Testing with swizzle...")
        test_4wave_scheduled[TEST_M, TEST_N, TEST_K, enable_swizzle=True](ctx)
