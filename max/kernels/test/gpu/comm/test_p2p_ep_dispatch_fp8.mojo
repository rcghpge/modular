...
fn test_dispatch[
    ...
    var device_output_bufs_list = List[DeviceBuffer[fp8_dtype]](capacity=n_ranks)
    var device_output_scales_bufs_list = List[DeviceBuffer[scales_dtype]](capacity=n_ranks)
    ...
    for i in range(n_ranks):
        ...
        device_output_bufs_list.append(ctx.enqueue_create_buffer[fp8_dtype](n_slots * max_recv_num_tokens * hidden_size))
        device_output_scales_bufs_list.append(ctx.enqueue_create_buffer[scales_dtype](n_slots * max_recv_num_tokens * hidden_size // group_size))
    ...
    comptime output_scales_layout = Layout.row_major(
        hidden_size // group_size, n_tokens_per_rank * n_ranks * n_local_experts
    )
    ...
    @always_inline
    @parameter
    fn get_output_tensor(dev_idx: Int, slot_idx: Int, out result: LayoutTensor[fp8_dtype, output_layout, MutAnyOrigin]) raises:
        return type_of(result)(
    ...
    @always_inline
    @parameter
    fn get_output_scales_tensor(dev_idx: Int, slot_idx: Int, out result: LayoutTensor[scales_dtype, output_scales_layout, MutAnyOrigin]) raises:
        return type_of(result)(
            device_output_scales_bufs_list[dev_idx].unsafe_ptr() + slot_idx * max_recv_num_tokens * hidden_size // group_size,
            RuntimeLayout[output_scales_layout].row_major(
                IndexList[2](hidden_size // group_size, max_recv_num_tokens)
            )
        )
    ...
    fn verify_results(dev_idx: Int) raises:
        ...
        var host_output_scales = alloc[Scalar[scales_dtype]](
            n_slots * max_recv_num_tokens * hidden_size // group_size
        )
        ...
        ctx.enqueue_copy(
            host_output_scales, device_output_scales_bufs_list[dev_idx]
        )
        ctx.enqueue_copy(
        ...
        for slot_idx in range(n_slots):
            ...
            var slot_output_scales = (
                host_output_scales
                + slot_idx * max_recv_num_tokens * hidden_size // group_size
            )
            ...
            for expert_idx in range(n_local_experts):
                ...
                for token_idx in range(
                    ...
                    # (with FP8 quantization tolerance)
                    for i in range(hidden_size):
                        var remote_token_val = remote_rank_input_tokens[
                            remote_loc * hidden_size + i
                        ]
                        var curr_fp8_val = slot_output[
                            token_idx * hidden_size + i
                        ]
                        var curr_token_scale = slot_output_scales[
                            (i // group_size) * max_recv_num_tokens + token_idx
                        ]
                        var curr_token_val = (
...
def main():
    ...
    comptime fp8_dtype = DType.float8_e4m3fn if has_nvidia_gpu_accelerator() else DType.float8_e4m3fnuz
