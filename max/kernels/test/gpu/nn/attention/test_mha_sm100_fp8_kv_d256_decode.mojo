...
"""Bit-exact-ish correctness test for SM100 MHA with FP8 KV cache — DECODE.

Decode-only sibling of `test_mha_sm100_fp8_kv_d256_prefill.mojo`. Both
files were split out of the original `test_mha_sm100_fp8_kv_d256.mojo`
so the prefill and decode paths can advance independently.

At `seq_len=1` with head_dim=256 on B200, `flash_attention` lands in
`mha_1q.mojo` (the production SM100 1Q decode kernel). That kernel is
slated for removal; until its successor lands, these decode cases run
through the existing two-launch dequant-staging path
(`dequant_paged_fp8_kv_to_bf16` + bf16 attention) — same coverage as
before, the file is just separated so it can be disabled or retargeted
independently of the prefill gate.

Acceptance bar: cosine ≥ 0.9995 between bf16-reference and
dequant-via-fp8 outputs.
...
def _cosine_similarity(
    ...
    """Host-side cosine similarity over two bf16 buffers."""
    var dot: Float64 = 0.0
    var aa: Float64 = 0.0
    var bb: Float64 = 0.0
    for i in range(n):
...
def _quantize_bf16_to_fp8_blockwise[
    ...
    """Per-row, per-`granularity`-block symmetric max-scale quantization.

    For each row × each contiguous `granularity`-element block along the
    head_dim, compute:
        scale = block_absmax / fp8_e4m3_max  (with floor at 1e-12)
        fp8[i] = clip(round(bf16[i] / scale))

    The dequant kernel inverts this with `bf16(float(fp8) * scale)`.
    """
    comptime fp8_max = Float32(448.0)  # e4m3 finite max
...
def execute_fp8_kv_test[
    ...
    *,
    ...
    Both paths run through `flash_attention` (SM100 dispatch). At
    seq_len=1 with head_dim=256 this lands in `mha_1q.mojo`, exercising
    the production decode kernel against the dequant round trip.
    """
    comptime assert seq_len == 1, (
        "Decode test file must run with seq_len == 1. Use the _prefill"
        " test file for seq_len > 1 cases (which route through"
    ...
    print(
        "test_mha_sm100_fp8_kv_d256_decode: ",
        "mask=",
        mask_name,
        " group=",
        group,
    ...
    # ---- Host: random bf16 K, V tensors in BSHD layout ----
    ...
    # ---- Host: quantize K,V to fp8 + fp32 scales per (token, head, block) ----
    ...
    var used = Set[Int]()
    # Simple sequential assignment: K gets [0, 1, ..., n-1], V gets [n, n+1, ...].
    # This deliberately mixes even/odd parities across K and V.
    ...
    # Copy contiguous BSHD K (and V) into the paged buffers at the LUT-selected
    # physical blocks.
    ...
    # ---- Device: copy paged fp8 + scales, allocate bf16 paged shadow ----
    ...
    # ---- Copy dequant result back and rebuild a contiguous BSHD bf16 K,V ----
    ...
    # ---- Run reference attention on bf16-original and dequant-bf16 ----
...
def main() raises:
    with DeviceContext() as ctx:
        ...
        # Decode-shape cases (seq_len=1) routed through the SM100 dispatch.
        # At seq_len=1 with head_dim=256 on B200, `flash_attention` lands in
        # `mha_1q.mojo`, exercising the production decode kernel against
        # the dequant round trip. The 1Q kernel is slated for removal —
        # these cases will retarget the successor kernel when it lands.
        execute_fp8_kv_test[
            CausalMask,
            num_q_heads=32,
            group=8,
        ...
        # Production Gemma4-sliding decode shape: n_q_heads=32, n_kv_heads=16
        ...
        print("test_mha_sm100_fp8_kv_d256_decode: ALL PASSED")
