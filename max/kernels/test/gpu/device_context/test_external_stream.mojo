...
from std.memory._nonnull import bitcast, unsafe_origin_cast
...
def native_stream_ptr(
    stream: DeviceStream,
) raises -> _CPointer[NoneType, MutAnyOrigin]:
    comptime if has_nvidia_gpu_accelerator():
        return unsafe_origin_cast[MutAnyOrigin](bitcast[NoneType](CUDA(stream)))
    else:
        return unsafe_origin_cast[MutAnyOrigin](bitcast[NoneType](HIP(stream)))
