...
def matrix_mul_tiled_boundary_kernel(
    ...
    # Allocate shared memory using external_memory (similar to working fig5_10.mojo)
    var Mds = rebind[
        UnsafePointer[
        ...
        external_memory[
            ...
            address_space=AddressSpace.SHARED,
            alignment=16,
            name="shared_boundary_memory",
        ]()
    )
    var Nds = Mds + TILE_WIDTH * TILE_WIDTH
...
def matmul_tiled_boundary(
    ...
    # Launch kernel with shared memory configuration (following working fig5_10.mojo pattern)
