...
- Added new Mojo Manual documentation for `TileTensor`, including an updated
  [tile `Layout` guide](/docs/manual/tile-tensor/layouts/), a
  [`TileTensor` guide](/docs/manual/tile-tensor/tensors/) with examples, and an
  [introduction](/docs/manual/tile-tensor/) describing the relationship between
...
- The [`ImplicitlyCopyable`](/docs/std/builtin/value/ImplicitlyCopyable/),
...
- The `__init__` method required by the
  [`Movable`](/docs/std/builtin/value/Movable/) trait has had its named argument
  changed from `take` to `move`. Explicitly calling a move initializer is now
  `SomeObject(move=)` instead of `SomeObject(take=)`.
...
- Added the [`UntrackedOrigin`](/docs/std/builtin/type_aliases/#untrackedorigin)
  ...
  [`LayoutTensor`](/docs/layout/layout_tensor/LayoutTensor/) have
  correspondingly been renamed from `as_any_origin()` to
...
- The core collection types no longer require their element type to be
  ...
  [`InlineArray[T, size]`](/docs/std/collections/inline_array/InlineArray/),
...
- `String.as_bytes_mut()` has been renamed to `String.unsafe_as_bytes_mut()`, to
  ...
  [`Span[Byte]`](/docs/std/memory/span/Span/) can lead to later issues like out
  of bounds access.
...
- Added
  [`DeviceContextList[size]`](/docs/std/gpu/host/device_context/DeviceContextList/)
  in `std.gpu.host`: a fixed-size,
  `Copyable`/`ImplicitlyCopyable`/[`Sized`](/docs/std/builtin/len/Sized/)
  collection of
  [`DeviceContext`](/docs/std/gpu/host/device_context/DeviceContext/) values.
...
- Added
  [`std.gpu.host.CompletionFlag`](/docs/std/gpu/host/device_context/CompletionFlag/),
...
- Added a [`DevicePointer`](/docs/std/gpu/host/device_context/DevicePointer/)
...
- [`Coord`](/docs/std/utils/coord/Coord/),
  [`coord()`](/docs/std/utils/coord/coord-function/), `Idx`, `ComptimeInt`,
  ...
  layout and kernel code; `layout` also hoists the common names at package scope
  for convenience.
...
- Added nested-layout support (CuTe layout algebra) to
  [`Layout`](/docs/layout/layout/Layout/) and
