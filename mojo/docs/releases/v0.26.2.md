...
- The
  [`DimList`](https://docs.modular.com/max/api/kernels/buffer/dimlist/DimList/)
  type has moved to representing its dimensions as parameters to the type
  instead of values inside the type, directly reflecting that the dimensions are
  known at compile time. Change `DimList(x, y)` to `DimList[x, y]()`.
