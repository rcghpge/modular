...
def kernel():
    # This call does not work on Apple Silicon
...
On Apple Silicon, `print()` isn't available inside GPU kernels. But you
can still say hello.

Instead of printing from the kernel, write a value to device memory and
...
While `print()` isn't supported inside Apple Silicon GPU kernels,
the final report runs on the CPU and is safe to run.
