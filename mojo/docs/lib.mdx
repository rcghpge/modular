...
import {useSidebarCategory} from '@site/src/util/sidebarHelpers';
...
export const StdLibCards = () => {
  const stdLib = useSidebarCategory('Standard library');
  if (!stdLib) return null;
  // Don't show the index page as a card.
  const items = stdLib.items.filter(
    item => !(item.type === 'link' && item.docId === 'mojo/std/index')
  );
...
The layout library allows you to describe the memory layout of multidimensional
data and build high-performance GPU kernels on top of those layouts. It maps
multidimensional coordinates to linear offsets in memory and provides tensor
views that support tiling, vectorization, and distribution of work across GPU
threads—all without copying the underlying data. To learn more, see
[Introduction to layouts](/mojo/manual/layout/layouts) and [Using
LayoutTensor](/mojo/manual/layout/tensors).

Top-level packages:

export const LayoutCards = () => {
  const layout = useSidebarCategory('Layout library');
  if (!layout) return null;
  // Don't show the index page as a card.
  const items = layout.items.filter(
    item => !(item.type === 'link' && item.docId === 'mojo/layout/index')
  );
  return <DocCardList items={items} />;
};
...
operations](/max/develop/custom-ops/) or standalone [GPU
kernels](/mojo/manual/gpu/basics) that are portable across CPUs and GPUs.

Top-level packages:

export const KernelsCards = () => {
  const kernels = useSidebarCategory('MAX AI kernels');
  return kernels ? <DocCardList items={kernels.items} /> : null;
};
...
export const DecoratorCards = () => {
  const decorators = useSidebarCategory('Decorators');
  return decorators ? <DocCardList items={decorators.items} /> : null;
};
