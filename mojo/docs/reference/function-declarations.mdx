...
description: >
  ...
  signatures, passing conventions, markers, variadic
...
<!-- NOT VERIFIED:
"Copy constructors can't raise" — likely enforced via trait/IR; no specific
...
Declares that a function type doesn't capture values from its defining
...
:::note
`RegisterPassable` types can't define a move constructor.
...
6. Pick the candidate that is a non-`@implicit` constructor
...
As a result, when using a trivial register type like `Int`, `Float64`, or
similar, `(c^).which()` becomes `c.which()`, so `ref self` always wins. The
compiler will warn you but won't reject your code.
...
- Don't rely on two `@implicit` constructors being reachable from the
  same source type. Cast at the call site or make one constructor
  non-`@implicit`.
