...
[`Powable`](/mojo/std/math/math/Powable) requires `__pow__()`, doesn't
provide a default.
...
[`Equatable`](/mojo/std/builtin/comparable/Equatable) provides
`__eq__()` if all your struct's fields conform to `Equatable` using
pairwise field comparison. `__ne__()` derives from `__eq__()`.

[`Comparable`](/mojo/std/builtin/comparable/Comparable) provides
...
[`Identifiable`](/mojo/std/builtin/identifiable/Identifiable) requires
