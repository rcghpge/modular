...
:::note Sets are not initializer lists
...
The walrus operator introduces a new binding in a subordinate scope, such
as an `if` clause, `for` loop, or `while` loop. It has the lowest
precedence of any expression operator. Use parentheses to avoid ambiguity
when needed.
