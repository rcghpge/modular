...
struct Node:
    var value: String
    var next: Optional[Node]  # Error because this is a recursive reference
```
...
```mojo
@fieldwise_init
struct Pair[T: Copyable & ImplicitlyDeletable]:
    var first: Self.T
    var second: Self.T
```
...
`ImplicitlyDeletable` when all members are also implicitly deletable.
With generics, the parameter's traits must include `ImplicitlyDeletable`
for this to apply:
...
Without `ImplicitlyDeletable`, the compiler can't verify that the struct
...
# Error: trait method requirement 'foo' has conflicting default
# implementations in TraitA and TraitB; you must implement it manually
```
...
```mojo
@fieldwise_init
struct NotWritable(ImplicitlyCopyable & ImplicitlyDeletable):
    var item: Int
...
Implicitly deletable structs get a default `__del__()`. A custom `__del__()`
