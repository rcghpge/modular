...
<!-- VERIFIED: From parseCaptureList in Signatures.cpp:1274-1364 -->
...
def name(argument-list) register_passable {capture-list}
    -> ReturnType:
    body
-->
...
<!-- VERIFIED: parseCaptureList, Signatures.cpp:1274-1364 -->
...
register-passable name (like a `read` parameter of type `Int`)
...
error: register passible value 'x' can not be captured by
       'mut'. Do you mean 'read'?

error: register passible value 'x' can not be captured by
       'ref'. Do you mean 'read'?
```
-->
...
- `{read name^}`, `{mut name^}`, and `{ref name^}` are
  rejected.
- A bare `name^` is equivalent to `var name^`. Both produce the
  same `kConventionMove` entry. The compiler accepts both forms,
  but `mojo format` currently rejects the bare form. Use
  `{var name^}` in code that must round-trip through the
  formatter.

## Copyable closures: `var^`

<!-- VERIFIED: unified_closures_mem.mojo:94-103,
     unified_closures.mojo:185, Signatures.cpp:1278-1293 -->
...
<!-- VERIFIED: parseCaptureList loop, Signatures.cpp:1352-1362 -->
...
<!--
Removed for now:
`register_passable` requires every captured value to be itself
register-passable, or to be captured by reference (`read`,
`mut`, `ref`). Capturing a `String` or `List` by `var` in a
`register_passable` closure is rejected.
-->
...
<!-- Removed for now:
| Mutable or reference capture of immutable register-passable value                 | `{mut name}` or `{ref name}` on an immutable register-passable outer binding |
-->
