...
- `is_compile_time()` has been renamed to `__is_run_in_comptime_interpreter`,
  ...
  versus runtime generated code. This value cannot be used as a constant in
  any comptime expressions because it is always evaluated as `True` for
  comptime expressions. The check should be used as a condition for runtime
  `if`. The compiler now warns if it is used as a condition for `comptime if`.
