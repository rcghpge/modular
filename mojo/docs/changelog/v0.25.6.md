...
- The following Mojo standard library types are no longer implicitly copyable:
  ...
  [`BitSet`](/mojo/std/collections/bitset/BitSet/), `UnsafeMaybeUninitialized`,
  `DLHandle`, [`BenchConfig`](/mojo/std/benchmark/bencher/BenchConfig),
...
- The
  [`compile.reflection.get_type_name()`](/mojo/std/reflection/type_info/get_type_name/)
...
- `mojo doc --validate-doc-strings` now emits a warning when an `fn` function is
  ...
  [raising functions](/mojo/manual/functions#raising-and-non-raising-functions),
