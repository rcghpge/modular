...
fault. You can disable this by adding `stack-trace-on-crash = false` under the
`[max-debug]` section of your `modular.cfg`.
...
`stack-trace-on-error`, or configure the `max-debug.stack-trace-on-error`
Config key directly.
