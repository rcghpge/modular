...
When calling raising functions or other `Error`-raising code from a function that
...
### Functions can catch typed errors

A function can call a function that raises a typed error and catch it
with full field access:

```mojo
def error_caller() raises:
    try:
        _ = validate_typed(-5)
    except e:
...
However, if a function calls code that raises a typed error without
...
Calling [`close()`](/mojo/std/io/file/FileHandle#close) releases the
...
possibility that [`read()`](/mojo/std/io/file/FileHandle#read) could
...
However, the [`FileHandle`](/mojo/std/io/file/FileHandle) struct
returned by [`open()`](/mojo/std/io/file/open) is a context manager.
...
[reflection](/mojo/manual/reflection/) to inspect the error
