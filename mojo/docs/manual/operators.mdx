...
This section lists the operators that Mojo supports, their order of precedence
...
`Int` and `UInt` support all arithmetic operators except matrix
multiplication (`@`), along with all bitwise and shift operators.

If both operands are `Int`, the result is `Int`. If both are `UInt`, the
result is `UInt`. If one is `Int` and the other `UInt`, the result is
`Int`.

For division, `/` truncates toward zero and `//` rounds toward negative
infinity. Both return an integer of the same type as the operands—neither
...
quotient_int = a_int // b_int # rounds towards negative infinity
print("Int quotient:", quotient_int)
quotient_uint = i_uint / j_uint # truncates towards zero
...
- `%`, the modulo operator, returns the remainder after dividing the numerator
  ...
  `//` and `%` operators can be defined as
  `num == denom * (num // denom) + (num % denom)`. For example:
...
Some types have *identities* that can be tested with `is` and `not is`.
...
`Boolable` types adopt a kind of *truthiness*. You
...
When a type conforms to
[`Identifiable`](/mojo/std/builtin/identifiable/Identifiable), these operators
call the [`__is__()`](/mojo/std/builtin/identifiable/Identifiable/#__is__)
method to determine whether the left-hand operand has the same identity as the
right-hand operand. For example, you might compare raw values or use some other
way to establish what "identity" means in your work.

The `is not` operator uses the automatically provided
[`__isnot__()`](/mojo/std/builtin/identifiable/Identifiable/#__isnot__) method,
which returns the logical negation of `__is__()`.
...
[Writable](/mojo/std/format/Writable) trait so that we can use the `String()`
constructor to generate a `String` representation of a `Complex` value. You can
find out more about these traits and their associated methods in
[The `Writable` trait](/mojo/manual/traits#the-stringable-representable-and-writable-traits).
...
called `norm()` to calculate the
[Euclidean norm](https://en.wikipedia.org/wiki/Norm_(mathematics)#Euclidean_norm_of_complex_numbers)
