...
| `a if condition else b`| Ternary expression | Right to left |
...
In many languages, the ternary (`condition ? first : second`) provides a three-element operator that returns the first value if a Boolean expression is true, and the second if false:

```swift title="Not Mojo"
let result = score > 65 ? "pass" : "fail"
```

In Mojo, as in Python, the ternary operator takes a different approach:

```mojo title="Mojo"
result = "pass" if score > 65 else "fail"
```

The ternary can be used in expressions, assignments, and as function args:
...
When a type conforms to [`Identifiable`](/mojo/stdlib/builtin/identifiable/Identifiable),
these operators call the [`__is__()`](/mojo/stdlib/builtin/identifiable/Identifiable/#__is__)
...
[`__isnot__()`](/mojo/stdlib/builtin/identifiable/Identifiable/#__isnot__) method, which
...
[Writable](/mojo/stdlib/io/write/Writable) trait. While we're at it, let's
