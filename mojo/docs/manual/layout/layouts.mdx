...
Mojo's [`layout` package](/mojo/layout/) provides a number of APIs for
...
- The [`Layout`](/mojo/layout/layout/Layout) struct describes an
...
- [`LayoutTensor`](/mojo/layout/layout_tensor/LayoutTensor) is a
  flexible tensor type that combines a `Layout` and a pointer to data.
- The [`IntTuple`](/mojo/layout/int_tuple/IntTuple) struct is a
...
[public GitHub repo](https://github.com/modular/modular/tree/main/mojo/examples/layouts).
...
The [`print_layout()`](/mojo/layout/layout/print_layout) function
...
also provides an [`idx2crd()`](/mojo/layout/layout/Layout#idx2crd)
...
[`IntTuple`](/mojo/layout/int_tuple/IntTuple) type. Each element of an
...
The [`int_tuple`](/mojo/layout/int_tuple/) package provides a number of
functions for working with `IntTuple`. For example, it provides a
[`congruent()`](/mojo/layout/int_tuple/congruent) function for testing
...
[`row_major()`](/mojo/layout/layout/Layout/#row_major) and
[`col_major()`](/mojo/layout/layout/Layout/#col_major) static methods
are probably the simplest ways to create a layout. The `row_major()` method
creates a generalized row-major layout: that is, the rightmost coordinate varies
the fastest. The `col_major()` method creates a generalized column-major layout,
...
[`tile_to_shape()`](/mojo/layout/layout/tile_to_shape) function.
...
[`blocked_product()`](/mojo/layout/layout/blocked_product) function. The
...
[`make_ordered_layout()`](/mojo/layout/layout/make_ordered_layout)
