...
[`Coord`](/docs/std/utils/coord/Coord/) type, a tuple-like structure that
...
values using the [`coord()`](/docs/std/utils/coord/coord/) function:
...
You can mix compile-time and run-time values:
use a plain `Int` (or any `CoordLike`) for run-time values,
and [`Idx[N]`](/docs/std/utils/coord/Idx/) for compile-time integers.
...
The [`coord`](/docs/std/utils/coord/) module provides a number of
...
[`col_major_nested()`](/docs/layout/tile_layout/col_major_nested/). These
constructors compute row-major (or column-major) strides over the *flattened*
shape, then re-nest the strides to match the input shape.
...
use [`row_major()`](/docs/layout/tile_layout/row_major/) or
[`col_major()`](/docs/layout/tile_layout/col_major/) instead.

Once you have a nested layout, you can build a `TileTensor` from it and use
`tile()` to extract individual fragments. See
[Extracting a tile](/docs/manual/tile-tensor/tensors/#extracting-a-tile)
