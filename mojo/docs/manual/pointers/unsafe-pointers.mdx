...
```mojo
ptr = alloc[Int](10) # Allocate space for 10 Int values
```
...
does not _alias_ any existing value. (For example, it does not point to the
memory allocated for any other variable.) This memory isn't tracked by Mojo's
lifetime checker, so you're responsible for freeing it.
...
```mojo
str = "Hello, world!"
str_ptr = UnsafePointer(to=str)
opaque_ptr = str_ptr.bitcast[NoneType]()
