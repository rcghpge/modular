...
Initializer lists make it simple to construct instances without spelling out the type name.
For example, here's the standard way to call a function:
...
Because `process_pair` expects a `MyPair` instance, Mojo infers that
...
[Mutable arguments (`mut`)](https://docs.modular.com/mojo/manual/values/ownership/#mutable-arguments-mut)
:::
