...
- [Block-level synchronization and operations](#block-level-synchronization-and-operations):
  ...
  [`barrier()`](/mojo/std/gpu/sync/sync/barrier) and block reduction
  operations from the
  [`gpu.primitives.block`](/mojo/std/gpu/primitives/block) module.
...
- [`shuffle_down(value, offset)`](/mojo/std/gpu/primitives/warp/shuffle_down):
  ...
  from lane current_lane + `offset`). If the resulting lane ID is greater than or
...
- **Semaphores**
  ...
  [`state()`](/mojo/std/gpu/sync/semaphore/Semaphore/#state) methods for
