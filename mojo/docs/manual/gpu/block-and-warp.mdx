...
[`TileTensor`](/mojo/layout/tile_tensor/TileTensor/) and
[`stack_allocation()`](/mojo/layout/tile_tensor/stack_allocation/) to allocate
shared memory tiles.

{/* TODO(DOCS-1656/DOCS-1658/PR#82292): once Arthur's tile-tensor manual
    section merges, uncomment the sentence below. Pick the final URL/anchor
    based on what Arthur ships — most likely
    /mojo/manual/tile-tensor/tensors (general background) or
    /mojo/manual/tile-tensor/tensors#creating-a-tiletensor-in-shared-or-local-memory
    (the specific pattern used in the example below). */}
{/*
For an introduction to `TileTensor` itself—including how to create one in
global, shared, or local memory—see
[Using TileTensor](/mojo/manual/tile-tensor/tensors).
*/}
...
- [Block-level synchronization and operations](#block-level-synchronization-and-operations):
  ...
  [`barrier()`](/mojo/std/gpu/sync/sync/barrier) and block reduction operations
  from the [`gpu.primitives.block`](/mojo/std/gpu/primitives/block) module.
- [Warp-level operations](#warp-level-operations): How to perform fine-grained
  synchronization with [`syncwarp()`](/mojo/std/gpu/sync/sync/syncwarp) and
  leverage high-speed data exchange using
  [`gpu.primitives.warp`](/mojo/std/gpu/primitives/warp) primitives.
...
The [`gpu.sync.barrier()`](/mojo/std/gpu/sync/sync/barrier) function is the
...
[`gpu.primitives.block`](/mojo/std/gpu/primitives/block) module. These
...
- [`sum(val)`](/mojo/std/gpu/primitives/block/sum): Computes the sum of `val`
  across all threads in the block.
- [`max(val)`](/mojo/std/gpu/primitives/block/max): Computes the maximum
  `val` across all threads in the block.
- [`min(val)`](/mojo/std/gpu/primitives/block/min): Computes the minimum
  `val` across all threads in the block.
- [`broadcast(val, src_thread=0)`](/mojo/std/gpu/primitives/block/broadcast):
  Broadcasts the value from `src_thread` to all other threads in the block.
- [`prefix_sum[exclusive=False](val)`](/mojo/std/gpu/primitives/block/prefix_sum):
...
The [`gpu.sync.syncwarp()`](/mojo/std/gpu/sync/sync/syncwarp) function
...
The [`gpu.primitives.warp`](/mojo/std/gpu/primitives/warp) module provides
five shuffle primitives, each optimized for specific data movement patterns:

- [`shuffle_up(value, offset)`](/mojo/std/gpu/primitives/warp/shuffle_up):
...
- [`shuffle_down(value, offset)`](/mojo/std/gpu/primitives/warp/shuffle_down):
...
- [`shuffle_xor(value, offset)`](/mojo/std/gpu/primitives/warp/shuffle_xor):
...
- [`shuffle_idx(value, offset)`](/mojo/std/gpu/primitives/warp/shuffle_idx):
...
- [`broadcast(value)`](/mojo/std/gpu/primitives/warp/broadcast): A
...
- [`max(value)`](/mojo/std/gpu/primitives/warp/max): Computes the maximum
  `value` across all threads in the warp. The result is broadcast to all lanes.

- [`min(value)`](/mojo/std/gpu/primitives/warp/min): Computes the minimum
  `value` across all threads in the warp. The result is broadcast to all lanes.

- [`sum(value)`](/mojo/std/gpu/primitives/warp/sum): Computes the sum of
  `value` across all threads in the warp. The result is broadcast to all lanes.

- [`prefix_sum[exclusive=False](value)`](/mojo/std/gpu/primitives/warp/prefix_sum):
  Computes an inclusive (default) or exclusive prefix sum (scan) across threads
...
- **Semaphores**
  ([`gpu.sync.semaphore.Semaphore`](/mojo/std/gpu/sync/semaphore/Semaphore),
  [`gpu.sync.semaphore.NamedBarrierSemaphore`](/mojo/std/gpu/sync/semaphore/NamedBarrierSemaphore)):
  Device-wide semaphore implementations for inter-CTA synchronization using
  shared lock variables. Provides
  [`fetch()`](/mojo/std/gpu/sync/semaphore/Semaphore/#fetch),
  [`wait()`](/mojo/std/gpu/sync/semaphore/Semaphore/#wait),
  [`release()`](/mojo/std/gpu/sync/semaphore/Semaphore/#release), and
  [`state()`](/mojo/std/gpu/sync/semaphore/Semaphore/#state) methods for state
  management methods for coordinating work across thread blocks.

- **Named barriers**
  ([`gpu.sync.named_barrier()`](/mojo/std/gpu/sync/sync/named_barrier),
  [`gpu.sync.named_barrier_arrive()`](/mojo/std/gpu/sync/sync/named_barrier_arrive)):
...
- **Memory barriers**: A suite of functions for tracking asynchronous memory
  ...
  [`gpu.sync.mbarrier_init()`](/mojo/std/gpu/sync/sync/mbarrier_init),
  ([`gpu.sync.mbarrier_arrive()`](/mojo/std/gpu/sync/sync/mbarrier_arrive),
  [`gpu.sync.mbarrier_arrive_expect_tx_shared()`](/mojo/std/gpu/sync/sync/mbarrier_arrive_expect_tx_shared),
  [`gpu.sync.mbarrier_arrive_expect_tx_relaxed()`](/mojo/std/gpu/sync/sync/mbarrier_arrive_expect_tx_relaxed),
  [`gpu.sync.mbarrier_test_wait()`](/mojo/std/gpu/sync/sync/mbarrier_test_wait),
  and
  [`gpu.sync.mbarrier_try_wait_parity_shared()`](/mojo/std/gpu/sync/sync/mbarrier_try_wait_parity_shared)).

- **Thread fence**
  ([`gpu.intrinsics.threadfence()`](/mojo/std/gpu/intrinsics/threadfence)):
...
- **Async bulk copy synchronization**
  ([`gpu.sync.cp_async_bulk_commit_group()`](/mojo/std/gpu/sync/sync/cp_async_bulk_commit_group),
  [`gpu.sync.cp_async_bulk_wait_group()`](/mojo/std/gpu/sync/sync/cp_async_bulk_wait_group)):
...
- **Schedule barriers**
  ([`gpu.sync.schedule_barrier()`](/mojo/std/gpu/sync/sync/schedule_barrier),
  [`gpu.sync.schedule_group_barrier()`](/mojo/std/gpu/sync/sync/schedule_group_barrier)):
...
- **Wait count**
  ([`gpu.sync.s_waitcnt()`](/mojo/std/gpu/sync/sync/s_waitcnt),
  [`gpu.sync.s_waitcnt_barrier()`](/mojo/std/gpu/sync/sync/s_waitcnt_barrier)):
...
[`Atomic`](/mojo/std/os/atomic/Atomic/) operations for simple updates. For
example, you could use the
[`Atomic.fetch_add()`](/mojo/std/os/atomic/Atomic/#fetch_add) method to
...
[`gpu.WARP_SIZE`](/mojo/std/gpu/globals/#warp_size) constant to ensure your
