...
We can pass this function as a parameter to `enqueue_function_checked()` to
compile it for your attached GPU and launch it. First we need to get the
...
```mojo :clear
alias InTensor = LayoutTensor[dtype, layout, MutAnyOrigin]
```
...
ctx.enqueue_function_checked[multiply_kernel[2], multiply_kernel[2]](
    in_tensor,
    grid_dim=blocks,
    block_dim=threads
)
...
out_buffer.enqueue_fill(0)

alias out_layout = Layout.row_major(blocks)
alias OutTensor = LayoutTensor[dtype, out_layout, MutAnyOrigin]
...
ctx.enqueue_function_checked[reduce_sum, reduce_sum](
    in_tensor,
    out_tensor,
    grid_dim=blocks,
    block_dim=threads
