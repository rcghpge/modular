...
3. Verify the project is configured correctly by checking the version of Mojo
...
The [`DeviceContext`](/mojo/std/gpu/host/device_context/DeviceContext/) type
...
Use the
[`DeviceContext()`](/mojo/std/gpu/host/device_context/DeviceContext#__init__)
...
[`has_accelerator()`](/mojo/std/sys/info/has_accelerator/) function to check
...
Modify the `main()` function and add `has_apple_gpu_accelerator()` to the list
of imports from the `sys` module so that your program looks like this:
...
:::caution If you're completing this tutorial on macOS

Printing from within a kernel function is not currently supported on Apple
silicon GPUs. If you run this program on macOS, you'll see only the following
output:
...
If you're completing this tutorial on macOS, go ahead and read the rest of this
step and the next step to learn the concepts of compiling and running a kernel.
You'll be able to continue the hands-on portion of the tutorial starting with
[6. Allocate host memory for the input vectors](#6-allocate-host-memory-for-the-input-vectors),
where we'll begin writing a different kernel that doesn't use `print()`.
...
The
[`enqueue_function_checked()`](/mojo/std/gpu/host/device_context/DeviceContext#enqueue_function_checked)
...
`DeviceContext` also includes
[`enqueue_function_checked()`](/mojo/std/gpu/host/device_context/DeviceContext/#enqueue_function_checked)
and
[`compile_function_checked()`](/mojo/std/gpu/host/device_context/DeviceContext/#compile_function_checked)
...
currently require the kernel function to be provided *twice* as parameters. This
requirement will be removed in a future API update, when typechecking will
become the default behavior for both `enqueue_function_checked()` and
...
[`HostBuffer`](/mojo/std/gpu/host/device_context/HostBuffer) type to
...
The [`DeviceBuffer`](/mojo/std/gpu/host/device_context/DeviceBuffer) type is
...
The [`enqueue_copy()`](/mojo/std/gpu/host/device_context/DeviceContext#enqueue_copy)
...
isn't strictly necessary for this example, we'll use `LayoutTensor` so that
...
As a convenience, the [`gpu`](/mojo/std/gpu) package includes a `global_idx`
...
The `enqueue_function_checked()` method enqueues the compilation and invocation
...
export const Examples = {
    ...
    metadata: {
        permalink: "https://github.com/modular/modular/tree/main/mojo/examples/gpu-functions"
    }
};
