...
As you move from Python to Mojo, two skills are particularly
useful:

- **Mojo syntax** (`mojo-syntax`): keeps generated code aligned with the
  current language, even when older patterns look similar but
  no longer apply
- **Python interop** (`mojo-python-interop`): helps you call Python
  libraries correctly and bridge between Python and Mojo code

You describe the intent in Python terms. The skills guide the
translation into correct, modern Mojo.
