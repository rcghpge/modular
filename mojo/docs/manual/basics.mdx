...
```mojo
def greet(name: String) raises -> String:
    return "Hello, " + name + "!"
```
...
```mojo
def use_mypair() raises:
    var mine = MyPair(2, 4)
    mine.dump()
```

The `MyPair` struct contains two special methods, `__init__()`, the constructor,
and `__init__(out self, *, copy: Self)`, the copy constructor. _Lifecycle methods_
like this control how a struct is created, copied, moved, and destroyed.
