...
## Implicit destructors {#destructor}

Mojo calls a value's destructor after the value's last use unless a type
opts out of `ImplicitlyDeletable` destruction. The deletable trait provides
a default no-op destructor (`__del__()`), which you can override.

Create a custom `__del__()` implementation to perform any required cleanup.
This might include freeing dynamically allocated memory (for example,
through `UnsafePointer`) or releasing long-lived resources such as file
handles.

Types that are simple collections of other types usually don't need to
override `__del__()`.
...
Generic code parameterized over `AnyType` can accept both implicitly deletable
and explicitly destructible values. However, it cannot automatically destroy
...
def generic_function_2[T: ImplicitlyDeletable](var value: T):
...
- Require implicit destruction with the ImplicitlyDeletable trait, which
  guarantees that a `__del__()` destructor is available and can be called
  automatically:

  ```mojo
  def generic_function_3[T: ImplicitlyDeletable](var value: T):
      pass  # value.__del__() called automatically
  ```
