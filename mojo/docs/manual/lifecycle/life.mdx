...
variable to another, _without_ copying its fields. A value that's copyable
...
as `deinit`, it does _not_ call the destructor, since that would destroy
...
A move constructor is _not required_ to transfer ownership of a
