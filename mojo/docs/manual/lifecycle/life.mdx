...
[`Bool`](/mojo/std/builtin/bool/Bool), [`Int`](/mojo/std/builtin/int/Int),
and [`String`](/mojo/std/collections/string/string/String)—are defined as
...
into the new value. The `Copyable` trait also defines a default `copy()` method,
which provides a more user-friendly way to copy a value than invoking the
...
- The struct includes a non-owning type (like
  [`UnsafePointer`](/mojo/std/memory/unsafe_pointer/UnsafePointer)), and you
...
Notice that the copy constructor does not copy the `UnsafePointer` value (doing
...
fn copies():
    var a = HeapArray(2, 1)
    var b = HeapArray(copy=a)  # Calls the copy constructor
    a.dump()  # Prints [1, 1]
    b.dump()  # Prints [1, 1]
...
method. (It also gives you automatic `Movable` conformance.)
...
types are automatically movable, and **cannot** define a custom move
constructor.
...
the `name` value is being transferred from `take` to `self`. For
...
A critical feature of move constructors is that it takes the incoming value as
...
(during ownership transfer), the `take` argument is guaranteed to be a
...
[`OwnedPointer`](/mojo/std/memory/owned_pointer/OwnedPointer/) is an
...
As such, Mojo provides a trait `TrivialRegisterPassable` to declare these types of
values. Conforming to this trait tells Mojo that the
type should be copyable and movable, but that it has no user-defined logic for
this (no custom copy constructor or move constructor). It also tells Mojo to
pass the value in CPU registers whenever possible, which has clear performance
benefits.
