...
struct Struct(Writable):
    ...
    # comptime[T: Copyable & ImplicitlyDeletable & Writable] T' is not
...
- `conforms_to(T, Writable & ImplicitlyCopyable & ImplicitlyDeletable)`
  checks whether `T` satisfies the trait composition.
...
`ImplicitlyDeletable` constraint to keep things working.
...
- _Associated (dependent) types:_
  ...
  elements to be hashable or implicitly deletable.
...
beyond the base `Copyable` and `ImplicitlyDeletable`. Therefore
...
```mojo
struct SizedListWrapper[capacity: Int, T: Copyable & ImplicitlyDestructible](
