...
[`Defaultable`](/mojo/std/builtin/value/Defaultable), which requires a default
...
* [`Absable`](/mojo/std/builtin/math/Absable)
* [`AnyType`](/mojo/std/builtin/anytype/AnyType)
* [`Boolable`](/mojo/std/builtin/bool/Boolable)
...
* [`IntableRaising`](/mojo/std/builtin/int/IntableRaising)
* [`KeyElement`](/mojo/std/collections/dict/#keyelement)
* [`Movable`](/mojo/std/builtin/value/Movable)
* [`PathLike`](/mojo/std/os/pathlike/PathLike)
* [`Powable`](/mojo/std/builtin/math/Powable)
* [`Representable`](/mojo/std/builtin/repr/Representable)
...
* [`Roundable`](/mojo/std/builtin/math/Roundable)
...
The [`Sized`](/mojo/std/builtin/len/Sized) trait identifies types that
have a measurable length, like strings and arrays.

Specifically, `Sized` requires a type to implement the `__len__()` method.
This trait is used by the built-in [`len()`](/mojo/std/builtin/len/len)
...
The [`Intable`](/mojo/std/builtin/int/Intable) trait identifies a type that
can be converted to `Int`. The
[`IntableRaising`](/mojo/std/builtin/int/IntableRaising) trait describes a
...
The [`Stringable`](/mojo/std/builtin/str/Stringable) trait identifies a type
that can be explicitly converted to
[`String`](/mojo/std/collections/string/string/String). The
[`StringableRaising`](/mojo/std/builtin/str/StringableRaising) trait
...
[`format()`](/mojo/std/collections/string/string/String#format) method. These
traits require the type to define the
[`__str__()`](/mojo/std/builtin/str/Stringable#__str__) method.

In contrast, the [`Representable`](/mojo/std/builtin/repr/Representable)
trait defines a type that can be used with the built-in
[`repr()`](/mojo/std/builtin/repr/repr) function, as well as the `{!r}`
format specifier of the `format()` method. This trait requires the type to
define the [`__repr__()`](/mojo/std/builtin/repr/Representable#__repr__)
...
The [`Writable`](/mojo/std/io/write/Writable) trait describes a
type that can produce a human-readable text representation by writing to a
[`Writer`](/mojo/std/io/write/Writer) object. The [`print()`](/mojo/std/builtin/io/print)
function requires that its arguments conform to the `Writable` trait. This
enables efficient stream-based writing by default, avoiding unnecessary
intermediate String heap allocations.

The `Writable` trait requires a type to implement a
[`write_to()`](/mojo/std/io/write/Writable#write_to) method, which
is provided with an object that conforms to the
[`Writer`](/mojo/std/io/write/Writer) trait as an argument. You then
invoke the `Writer` instance's
[`write()`](/mojo/std/io/write/Writer#write) method to write a
...
[`String.write()`](/mojo/std/collections/string/string/String#write) static
...
### The `ImplicitlyDestructible` trait
...
The [`ImplicitlyDestructible`](/mojo/std/builtin/anytype/ImplicitlyDestructible)
trait represents a type with a destructor. Almost all traits inherit from
`ImplicitlyDestructible`, and all structs conform to `ImplicitlyDestructible` by
default. For any type that conforms to `ImplicitlyDestructible` and doesn't
define a destructor, Mojo generates a no-op destructor. This means you can call
the destructor on any type that inherits from `ImplicitlyDestructible`.
...
This trait exists to support a feature called *linear* or *explicitly-destroyed*
types. A linear type may not be destroyed by the compiler implicitly, and
instead may only be destroyed when the programmer explicitly calls a named
destructor method. The compiler will issue an error if a linear type becomes
unused without being explicitly destroyed.
...
The [`List`](/mojo/std/collections/list) type is an example of a
