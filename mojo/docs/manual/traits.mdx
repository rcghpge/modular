...
Traits _refine_ other traits. A trait that refines another trait includes all
...
The standard _register passable type_ has the following capabilities
and restrictions:
...
- Every field must either be register passable or trivially register
  passable.
- The type must be
  [`Movable`](/mojo/manual/lifecycle/life#move-constructor).
...
- _Arithmetic types_. This includes types such as `Int`, `Int32`,
  `Bool`, `Float64` etc.
- _Pointers_. The address value is trivial, not the data being pointed
  to.
- _Arrays of other trivial types_. `SIMD` is a good example.

These types are provided by Mojo's standard library.

A _trivially register passable type_ has the following capabilities
and restrictions:

- They do not use lifecycle methods.
- Every field must be trivially register passable.
- The type must be
  [`Copyable`](/mojo/manual/lifecycle/life#copy-constructor).
- They use a special trivial (no-op) destructor. They are _trivially
  movable_ (via `Copyable`, which refines `Movable`), _trivially
  copyable_, and _trivially destructible_.
...
The [`Writable`](/mojo/std/format/Writable) trait describes a type that can
produce a human-readable text representation by writing to a
[`Writer`](/mojo/std/format/Writer) object. The
[`print()`](/mojo/std/builtin/io/print) function requires that its arguments
...
[`write_to()`](/mojo/std/format/Writable#write_to) method. This method
receives a `mut writer: Some[Writer]` argument. A generic
[`Writer`](/mojo/std/format/Writer) that represents anything that can be
...
[`write_repr_to()`](/mojo/std/format/Writable#write_repr_to) method for
producing the "official" string representation of a type—the kind you would see
when calling [`repr()`](/mojo/std/builtin/repr/repr) or using the `{!r}`
