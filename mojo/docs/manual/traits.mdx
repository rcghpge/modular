...
`@register_passable("trivial")` decorator is deprecated. Please extend with
`TrivialRegisterType` trait instead.

`@register_passable` decorator is also deprecated. Please extend with
`RegisterType` trait instead.
...
* [`Writable`](/mojo/std/format/Writable)
* [`Writer`](/mojo/std/format/Writer)
...
The [`Writable`](/mojo/std/format/Writable) trait describes a
type that can produce a human-readable text representation by writing to a
[`Writer`](/mojo/std/format/Writer) object. The [`print()`](/mojo/std/builtin/io/print)
...
[`write_to()`](/mojo/std/format/Writable#write_to) method, which
is provided with an object that conforms to the
[`Writer`](/mojo/std/format/Writer) trait as an argument. You then
invoke the `Writer` instance's
[`write()`](/mojo/std/format/Writer#write) method to write a
...
[`write_string()`](/mojo/std/format/Writer#write_string) method) or other
...
- [`AnyType`](/mojo/std/builtin/anytype/AnyType): The base trait that all types
extend. Types conforming only to `AnyType` may require explicit destruction.

- [`ImplicitlyDestructible`](/mojo/std/builtin/anytype/ImplicitlyDestructible):
Types that can be automatically destroyed by calling `__del__()` when their lifetime ends.
