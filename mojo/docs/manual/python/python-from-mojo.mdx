...
- The `import_module()` method returns a reference to the module in the form of
  a [`PythonObject`](/mojo/std/python/python_object/PythonObject) wrapper. You
  ...
  See [Mojo wrapper objects](/mojo/manual/python/types#mojo-wrapper-objects) for
  more information about the `PythonObject` type.
...
reasons](/mojo/roadmap#the-standard-library-has-limited-exceptions-use).
...
[`add_to_path()`](/mojo/std/python/python/Python#add_to_path). For example,
