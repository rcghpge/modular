...
[`Writable`](/mojo/std/format/Writable) to be bound for use
...
[`PythonTypeBuilder.def_method()`](/mojo/std/python/bindings/PythonTypeBuilder/#def_method).
...
[`PythonTypeBuilder.def_staticmethod()`](/mojo/std/python/bindings/PythonTypeBuilder/#def_staticmethod).
...
struct Person(Writable):
    ...
    def write_to(self, mut writer: Some[Writable]):
        t"Person({self.name}, {self.age})".write_to(writer)
```
...
[Keyword arguments in Mojo](/mojo/manual/functions/#keyword-arguments) come in
two forms:
...
[`OwnedKwargsDict[PythonObject]`](/mojo/std/collections/dict/OwnedKwargsDict) as
