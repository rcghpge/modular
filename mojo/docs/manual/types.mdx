...
<figure id="table-3">
<figcaption><b>Table 3.</b> Mojo floating point types</figcaption>

| Type name | Description |
| --------- | ----------- |
| `Float16` | 16-bit floating point<br />(IEEE 754-2008 binary16) |
| `Float32` | 32-bit floating point<br />(IEEE 754-2008 binary32) |
| `Float64` | 64-bit floating point<br />(IEEE 754-2008 binary64) |
| `BFloat16` | 16-bit floating point<br />(16-bit version of IEEE 754 binary32) |
| `Float4_e2m1fn` | 4-bit floating point<br />(e2m1 format from Open Compute MX specification) |
| `Float8_e5m2` | 8-bit floating point<br />(OFP8 e5m2 format) |
| `Float8_e5m2fnuz` | 8-bit floating point<br />(AMD-only e5m2fnuz format - finite values and NaN only, no infinities) |
| `Float8_e4m3fn` | 8-bit floating point<br />(OFP8 e4m3fn format) |
| `Float8_e4m3fnuz` | 8-bit floating point<br />(AMD-only e4m3fnuz format - finite values and NaN only, no infinities) |
</figure>

:::note Floating Points, Exponents, and Mantissas
eXmX format (like `Float8_e5m2` and `Float8_e4m3fn` in table 3)
refers to the number of bits allocated to a floating
point number's exponent and mantissa.

All IEEE 754 floating point
use an implied leading 1. That means `Float32` is e8m23 but effectively e8m24,
`Float16` is e5m10 but effectively e5m11, `BFloat16` is e8m7 but
effectively e8m8, and `Float8_e4m3fn` is e4m3 but effectively e4m4.
:::

The types in tables 1, 2, and 3 are actually all aliases to a single type,
[`SIMD`](/mojo/std/builtin/simd/SIMD), which is discussed later.

All of the numeric types support the usual numeric and bitwise operators. The
[`math`](/mojo/std/math/) module provides a number of additional math
functions.
...
`Float32`, preserving its dynamic range, but uses only 7 bits for the mantissa (compared to Float32's 23 bits).
...
bandwidth is the primary bottleneck. The naming indicates bit allocation:
`e5m2` means 5 exponent bits and 2 mantissa bits. The `fnuz` suffix denotes
variants that are finite-only (no infinities) and have unsigned zero (no -0), used in AMD hardware.
...
`UInt64`, which are aliases to the [`SIMD`](/mojo/std/builtin/simd/SIMD)
type.
...
and another set representing the fraction or mantissa. Table 4 shows how each of
these types are represented in memory.

<figure>
<figcaption><b>Table 4.</b> Details of floating-point types</figcaption>
...
can also use the [`math.isclose()`](/mojo/std/math/math/isclose) function to
