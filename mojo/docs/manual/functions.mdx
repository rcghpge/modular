...
```mojo
def add(a: Int, b: Int) raises -> Int:
    return a+b
```
...
```mojo
def add_tensors[rank: Int](a: MyTensor[rank], b: MyTensor[rank]) raises -> MyTensor[rank]:
    # ...
```
...
- If a function doesn't return a value, you can either omit the return type or
  ...
  def greet(name: String) raises:
    print("Hello," name)

  def greet(name: String) raises -> None:
    print("Hello," name)
  ```
...
- If the function returns a value, you must either declare the return type using
  ...
  def incr(a: Int) raises -> Int:
    return a + 1

  def incr(a: Int, out b: Int) raises:
    b = a + 1
  ```
...
- The compiler doesn't allow a function declared with `fn` to raise an error
  ...
  the compiler assumes that _all_ functions declared with `def` _might_ raise an
...
```mojo
def myfunc(pos_only, /, pos_or_keyword, *, keyword_only) raises:
```
...
[`VariadicList`](/mojo/std/builtin/variadics/VariadicList).  Here is A
simple example:
...
```mojo
def make_worldly(mut *strs: String) raises:
    for ref i in strs:
        i += " world"
```
...
[generic](/mojo/manual/generics/), which requires using
...
```mojo
def count_many_things[*ArgTypes: Intable](*args: *ArgTypes) raises:
    ...
```
...
[`Intable`](/mojo/std/builtin/int/Intable) trait. The asterisk in `*ArgTypes`
...
[`VariadicPack`](/mojo/std/builtin/variadics/VariadicPack) because the
...
loop](/mojo/manual/metaprogramming/comptime-evaluation/#comptime-for):
...
```mojo
def get_greeting() raises -> String:
    return "Hello"
```
...
```mojo
def get_name_tag(var name: String, out name_tag: NameTag) raises:
...
```mojo
def get_name_tag(var name: String) raises -> NameTag:
    ...
def get_name_tag(var name: String, out name_tag: NameTag) raises:
    ...
```
...
def create_immovable_object(var name: String, out obj: ImmovableObject) raises:
...
```mojo
def create_immovable_object2(var name: String) raises -> ImmovableObject:
    obj = ImmovableObject(name^)
    obj.name += "!"
...
```mojo
def create_immovable_object3(var name: String) raises -> ImmovableObject:
...
The Mojo compiler _always_ treats a function declared with `def` as a _raising
function_, even if the body of the function doesn't contain any code that could
raise an error.
...
*typed errors*, where you specify a custom error type a function can raise:

```mojo
fn validate(value: Int) raises ValidationError -> Int:
    ...
```

Typed errors are only available with `fn` functions—`def` functions always use
the built-in `Error` type. For full details, see
[Typed errors](/mojo/manual/errors#typed-errors). For a comparison of how `fn`
and `def` handle errors differently, see
[`fn` vs `def` for error handling](/mojo/manual/errors#fn-vs-def-for-error-handling).
