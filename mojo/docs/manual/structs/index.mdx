...
- **Dunder methods** are named for their *d*ouble *under*-scored form, with
...
let you construct these values simply, relying on surrounding type context
...
Because `process_pair` expects a `MyPair` instance, Mojo infers the type from
...
This refactors as follows, with the keyword preserved:

```text
utility_function({0.5, fish="swordfish"})
```

Use initializer lists with already-declared instances, as Mojo knows
the variable's type:
...
on [copy constructors](/mojo/manual/lifecycle/life#copy-constructor).
