...
to 0. The expression `x >= 0` is called a _proposition_. With the exception
...
Mojo also supports _compile-time assertions_, which test a proposition at
...
- In the parameter list of a function or struct, to constrain the values that
  you can bind to one or more parameters.

  `def fib[x: Int where x >= 0]() -> Int:`

  A constraint can involve multiple parameters, as long as the `where` clause
  comes after the parameters:

  `def subspan[start: Int, end: Int where end > start](self) -> Self:`

- In a method declaration, to constrain the availability of the method
  based on parameters of the parent struct.
...
- In the trait conformance list for a struct, to declare that a struct conforms
  ...
  You'll use this form, called _conditional trait conformance_, when defining
  generic types. For details, see the section on
  [conditional trait conformances](/mojo/manual/generics/generics/#conditional-trait-conformance).
...
a _requirement_ that callers must prove, and `comptime assert` is one way to
...
If the constraint is _not_ understandable or _not_ provable by the
