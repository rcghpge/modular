...
to specify different passing conventions that define how values are passed into
functions.
...
made possible in Mojo through the use of passing conventions that ensure every
value has only one owner at a time.

A passing convention specifies whether an argument is mutable or immutable,
...
Mojo's default passing convention is similar in some ways to passing an
