...
```mojo
def print_list(list: List[Int]) raises:
    print(list.__str__())
...
def mutate_copy(l: List[Int]) raises -> List[Int]:
...
two variables have ownership of the same value. To fix the error, you must
...
```mojo
def add_to_list(var name: String, mut list: List[String]) raises:
    list.append(name^)
    # name is uninitialized, nothing to destroy

def consume_string(var s: String) raises:
    print(s)
    # s is destroyed here
```
