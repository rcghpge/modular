...
- Untracked origins. The untracked origins, `MutUntrackedOrigin` and
  `ImmutUntrackedOrigin` represent values that are not tracked by the lifetime
  checker, such as dynamically-allocated memory.
- Wildcard origins. The `ImmutUnsafeAnyOrigin` and `MutUnsafeAnyOrigin`
...
The untracked origins, `MutUntrackedOrigin` and `ImmutUntrackedOrigin` represent
...
The wildcard origins, `ImmutUnsafeAnyOrigin` and `MutUnsafeAnyOrigin`, are
