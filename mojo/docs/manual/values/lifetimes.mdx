...
- When working with types like
  [`Pointer`](/mojo/std/memory/pointer/Pointer) or
  [`Span`](/mojo/std/memory/span/Span) which are parameterized on the
...
Or you can use the [`Origin`](/mojo/std/builtin/type_aliases/Origin)
...
[`Pointer`](/mojo/std/memory/pointer/Pointer) to an existing value, but
...
[`as_string_slice()`](/mojo/std/builtin/string_literal/StringLiteral#as_string_slice)
returns a [`StringSlice`](/mojo/std/collections/string/string_slice/StringSlice)
...
[`OwnedPointer`](/mojo/std/memory/owned_pointer/OwnedPointer): a smart
...
[`alloc()`](/mojo/std/memory/unsafe_pointer/alloc/) function returns an
...
```mojo
def pick_one(cond: Bool, ref a: String, ref b: String) raises -> ref[a, b] String:
