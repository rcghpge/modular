...
This example demonstrates how to use reflection to inspect a type at compile
time. It shows how to retrieve a type's name and iterate over a struct's fields,
including their names and types. This pattern is the foundation for most
reflection-based utilities, such as validation, copying, and equality checks.
...
runtime values. Knowing a value's type opens the door to retrieving
type-specific metadata such as field counts, names, and types. That metadata can
then be used to safely access and manipulate fields on concrete instances at
runtime, as long as the access itself is guided by compile-time information.

### Reflection calls

The reflection calls used in the following example include:

- `reflect[T]()`: Returns a `Reflected[T]` handle that exposes type and struct
  introspection through methods such as `name()`, `field_count()`,
  `field_names()`, and `field_types()`.

```mojo
def show_type[T: AnyType]():
    comptime r = reflect[T]()
    comptime type_name = r.name()         # name of the type
    comptime field_count = r.field_count() # count of fields
    comptime field_names = r.field_names() # indexed list of field names
    comptime field_types = r.field_types() # indexed list of field types
...
- When working with reflection, you must use parameterized types in the square
  ...
  `show_type[]()`. These elements can be concrete types like `MyStruct` or
...
- All processing of reflected information happens at compile time. For that
  reason, these reflection examples use
  [`comptime for`](/mojo/manual/metaprogramming/comptime-evaluation/#comptime-for)
  and
  [`comptime if`](/mojo/manual/metaprogramming/comptime-evaluation/#comptime-if).
  - In this example, `comptime for` allows the loop index to be used across
    iterations.
...
This example shows how reflected field information can drive real behavior. It
uses reflection to copy data from one instance to another by iterating over
fields and checking which ones are safe to copy. This pattern demonstrates how
reflection enables generic, type-safe operations without hardcoding field
access.
...
instance. For this method, you provide it with a target instance.
...
- As a method, `copy_to()` does not require a type parameter such as
  `copy_to[T]()`. It has direct access to `Self`, which is enabled by
  `MakeCopyable` trait adoption.
- The `copy_to()` _implementation_ is heavily parameterized and evaluated at
  compile time. It uses `comptime for` and `comptime if` together with
  reflection calls.
- The `r.field_ref[idx](self)` and `r.field_ref[idx](other)` calls return
  references to fields by index, which allows both reading from the source
...
This example demonstrates how reflection can be used to implement structural
equality. Compile-time loop unrolling, conformance checks, and reflection drive
runtime comparisons. This is the same pattern used for generic equality,
...
- As its name suggests, `Reflected[T].field_ref[]()` is limited to use
  with struct types.
...
- (Coming soon): Dive into generics, trait conformance tests with
  `conforms_to()`, and downcasts with `trait_downcast[]()`.
