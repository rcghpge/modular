...
- `struct_field_count[]()`, `struct_field_names[]()`, and
  `struct_field_types[]()`: Return the count, names, and types of a struct's
  fields.
...
```mojo
from std.reflection import struct_field_count, struct_field_ref, struct_field_types
...
- The `copy_to()` _implementation_ is heavily parameterized and evaluated at
  ...
  reflection calls.
- The `struct_field_ref[idx](self)` and `struct_field_ref[idx](other)` calls
  return references to fields by index, which allows both reading from the
  source instance and writing to the destination instance.
...
- As its name suggests, `struct_field_ref[]()` is limited to use with
  struct types.
