...
def _mutable_any_pointer(p: UnsafePointerV2[Int, MutAnyOrigin, **_]):
    assert_true(p)
    assert_equal(p[], 42)


def _immutable_any_pointer(p: UnsafePointerV2[Int, ImmutAnyOrigin, **_]):
    assert_true(p)
    assert_equal(p[], 42)
...
def test_unsafepointer_string():
    var nullptr = UnsafePointerV2[Int, MutOrigin.external]()
