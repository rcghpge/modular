...
[transformer](transformer.mdx) models. In self-attention, the queries, keys,
and values all come from the same sequence, which means every token attends to every other
token **in its own input**. This allows the model to build a rich understanding of
context by evaluating how each token relates to all others, regardless of their
