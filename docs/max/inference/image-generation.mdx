...
[`v1/responses`](/max/api/serve#operation/createResponse) endpoint to generate
images from text prompts or transform existing images, with examples for each
input type.

## Endpoint

The MAX [`v1/responses`](/max/api/serve#operation/createResponse) endpoint
...
| `residual_threshold` | `null`  | First-block cache (FBCache) reuse threshold (see [Cache backends](#cache-backends)) |
...
previous step when the first-block residual is similar enough. Tune the reuse
threshold per request with `provider_options.image.residual_threshold`. Lower
values skip fewer steps and preserve more detail; higher values skip more
aggressively. Leave it `null` to use the model-specific default.
