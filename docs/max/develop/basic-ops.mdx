...
[`max.experimental`](/max/api/python/experimental/) API. Be aware that you
cannot use these operations with [`Graph`](/max/api/python/graph/) objects
directly. See [Graph overview](/max/develop/graph) for more information.
...
multi-tensor operations, and operations with no tensor method equivalent:
...
When you call an operation like `F.relu(x)`, MAX doesn't execute it
immediately. Instead, MAX records nearby operations together and compiles them
as a group, which lets the compiler fuse and optimize them. This happens behind
the scenes; you write normal Python code and MAX handles the compilation and
execution for you.

This strategy is *lazy*: MAX defers work until your code actually needs a
concrete value. By waiting, MAX can see multiple operations at once and
optimize them together rather than executing each one individually.

MAX produces concrete values immediately when your code requires them:

- **Printing tensors**: `print(x)` needs real values to display.
- **Accessing scalar values**:
  [`item()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.item)
  returns a concrete Python number.
- **Indexing tensors**: `x[0]` extracts the actual value at that position.
- **Converting to other formats**: `np.from_dlpack(x)` requires a real memory
  buffer.
...
[`F.mean()`](/max/api/python/experimental.functional#max.experimental.functional.mean),
and
...
[`F.matmul()`](/max/api/python/experimental.functional#max.experimental.functional.matmul).
...
- **[Build a model graph with Module](/max/develop/modules)**: Use operations
in your model's `__call__()` method to define computation.
- **[Neural network modules](/max/api/python/nn)**: Explore pre-built layers in
