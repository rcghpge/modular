...
- **Tensor methods**: Call operations directly on
  ...
  like `x.sum()`, `x.reshape([2, 3])`, or `x.transpose(0, 1)`.
- **Functional API**: Call operations from
  [`max.experimental.functional`](/max/api/python/experimental.functional) that
  take your tensor as input, such as `relu(x)` or `concat([a, b])`. Use these
  for activation functions, multi-tensor operations, or explicit graph
  construction.
...
[`max.experimental.functional`](/max/api/python/experimental.functional).

Use functional operations (`F.*`) for:

- **Activation functions**: Operations like
  [`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu),
  [`F.sigmoid()`](/max/api/python/experimental.functional#max.experimental.functional.sigmoid),
  and
  [`F.tanh()`](/max/api/python/experimental.functional#max.experimental.functional.tanh)
  don't have tensor method equivalents.
- **Multi-tensor operations**: Operations that require multiple tensor inputs,
  like
  [`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat).
...
For more complex mathematical operations, MAX provides several approaches. In
this example, `abs()` finds the absolute value, and the `**` operator performs
exponentiation, which work seamlessly with tensors.
...
[`F.exp(x)`](/max/api/python/experimental.functional#max.experimental.functional.exp)
computes e^x where e is Euler's number (approximately 2.718), while
[`F.log(x)`](/max/api/python/experimental.functional#max.experimental.functional.log)
...
The
[`reshape()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.reshape)
method changes the shape of a tensor while preserving the total number of
elements. The following example transforms a 12-element vector into different
layouts—the total number of elements remains constant across all shapes:
...
The
[`transpose()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.transpose)
method swaps two dimensions of a tensor:
...
[`permute()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.permute)
to specify a new order for all dimensions. This is useful for converting between
different layout conventions. In the following example,
[`permute(0, 2, 1)`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.permute)
...
The
[`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat)
function joins multiple tensors along a specified dimension. This operation
requires the functional API since it operates on multiple tensors:
...
[`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat)
since there's no tensor method equivalent for multi-tensor operations.
...
- [`sum()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.sum):
  Sum of elements (tensor method)
- [`mean()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.mean):
  Average of elements (tensor method)
- [`max()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.max):
  Maximum value (tensor method)
- [`F.min()`](/max/api/python/experimental.functional#max.experimental.functional.min):
  Minimum value (functional API only)
...
[`F.sum(x)`](/max/api/python/experimental.functional#max.experimental.functional.sum).
Use these when building explicit graphs or when you prefer function-style
syntax.
...
# Create two matrices
x = Tensor([[1.0, 2.0], [3.0, 4.0]])  # 2x2
w = Tensor([[5.0, 6.0], [7.0, 8.0]])  # 2x2
...
[`F.matmul(x, w)`](/max/api/python/experimental.functional#max.experimental.functional.matmul).
...
The [`max.experimental.random`](/max/api/python/experimental.random) module
provides functions for creating tensors with random values. Random tensors are
essential for weight initialization and data augmentation.

### Create random values

[`random.uniform()`](/max/api/python/generated/max.experimental.random.uniform)
generates values uniformly distributed between `low` and `high`, while
[`random.normal()`](/max/api/python/generated/max.experimental.random.normal)
...
You can combine operations to implement neural network layers from scratch. The
following example shows a simple linear layer: the `linear_layer` function uses
the `@` operator for matrix multiplication and the `+` operator for bias
addition, while the activation step uses
[`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu)
from the functional API. Pre-built layers like
[`nn.Linear`](/max/api/python/generated/max.nn.Linear) work this way internally.
...
- **[Building graphs](/max/develop/get-started-with-max-graph-in-python)**: Use
  operations in explicit graph construction for production deployment.
