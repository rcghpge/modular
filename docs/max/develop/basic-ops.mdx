...
- **Tensor methods**: Call operations directly on [`Tensor`](/max/api/python/generated/max.experimental.tensor.Tensor) objects, like
  `x.sum(axis=0)`, `x.reshape([2, 3])`, or `x.transpose(0, 1)`.
- **Functional API**: Call operations from [`max.experimental.functional`](/max/api/python/experimental.functional)
  that take tensors as input, such as `F.relu(x)` or `F.concat([a, b])`.

## When to use the functional API

Tensor methods cover most arithmetic and shape operations. You'll need the
functional API for activation functions, multi-tensor operations, and
operations with no tensor method equivalent:

- **Activation functions**: [`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu),
  [`F.sigmoid()`](/max/api/python/experimental.functional#max.experimental.functional.sigmoid), and
  [`F.tanh()`](/max/api/python/experimental.functional#max.experimental.functional.tanh) don't have
  tensor method equivalents.
- **Multi-tensor operations**: [`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat) operates on multiple tensors at once.
- **Selected reductions**: [`F.min()`](/max/api/python/experimental.functional#max.experimental.functional.min) is functional-only; `sum`, `mean`, and `max` have both tensor method and functional forms.
...
[`F.sqrt()`](/max/api/python/experimental.functional#max.experimental.functional.sqrt)
...
[`reshape()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.reshape) changes the shape of a tensor while preserving the total number of
elements. The following example transforms a 12-element vector into three different
layouts:
...
[`transpose()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.transpose) swaps two dimensions of a tensor. The following example converts a 2×3 matrix to 3×2:
...
When you need to rearrange more than two dimensions, [`permute()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.permute) specifies a
new order for all dimensions as a list of indices. The following example
converts a `(batch, channels, length)` tensor to `(batch, length, channels)`:
...
[`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat) joins multiple tensors along a specified dimension. The following example concatenates two 2×2 matrices along each axis:
...
`sum(axis=0)` and `sum(axis=1)` return tensors with shape `[Dim(1), Dim(3)]`
and `[Dim(2), Dim(1)]` respectively, with the reduced dimension kept at size 1.
`F.min()` is functional-only; `sum`, `mean`, and `max` are also available as
[`F.sum()`](/max/api/python/experimental.functional#max.experimental.functional.sum),
[`F.mean()`](/max/api/python/experimental.functional#max.experimental.functional.mean), and
[`F.max()`](/max/api/python/experimental.functional#max.experimental.functional.max).
...
Matrix multiplication is also available as [`F.matmul()`](/max/api/python/experimental.functional#max.experimental.functional.matmul).

## Add activation functions

Activation functions are available through the functional API only.
[`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu) sets negative values to zero,
[`F.sigmoid()`](/max/api/python/experimental.functional#max.experimental.functional.sigmoid) maps values to (0, 1), and
[`F.tanh()`](/max/api/python/experimental.functional#max.experimental.functional.tanh) maps values to (-1, 1):
...
The [`max.experimental.random`](/max/api/python/experimental.random) module provides functions for creating tensors
with random values. [`random.uniform()`](/max/api/python/generated/max.experimental.random.uniform) generates values uniformly distributed
over a range; [`random.normal()`](/max/api/python/generated/max.experimental.random.normal) generates values from a Gaussian
distribution with the specified mean and standard deviation. Values vary each
run:
...
addition, and [`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu) for the activation step. Pre-built layers like
[`nn.Linear`](/max/api/python/generated/max.nn.Linear) work this way internally. Understanding the operations
gives you the foundation for custom layers when standard ones don't fit:
...
- **[Build a MAX graph](/max/develop/get-started-with-max-graph-in-python)**: Use these operations inside compiled graphs for production deployment.
- **[Neural network modules](/max/api/python/nn)**: Build models using `max.nn` with pre-built layers like `Linear`, `Conv2d`, and `ReLU`.
- **[Custom operations](/max/develop/custom-ops)**: Implement your own operations in Mojo when built-in operations don't meet your needs.
