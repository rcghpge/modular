...
- **[Build a MAX graph](/max/develop/get-started-with-max-graph-in-python)**:
  Use these operations inside compiled graphs for production deployment.
- **[Neural network modules](/max/api/python/nn)**: Build models using `max.nn`
  with pre-built layers like `Linear`, `Conv2d`, and `ReLU`.
