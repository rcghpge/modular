...
functions imported from [`max.experimental.functional`](/max/api/python/experimental.functional).

Use functional operations (`F.*`) for:

- **Activation functions**: Operations like [`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu),
...
- **Multi-tensor operations**: Operations that require multiple tensor inputs,
  like [`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat).
...
[`F.sqrt()`](/max/api/python/experimental.functional#max.experimental.functional.sqrt) uses the
...
[`F.exp(x)`](/max/api/python/experimental.functional#max.experimental.functional.exp) computes e^x where
e is Euler's number (approximately 2.718), while
[`F.log(x)`](/max/api/python/experimental.functional#max.experimental.functional.log) computes the
natural logarithm.
...
The [`reshape()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.reshape) method
...
The [`transpose()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.transpose) method
...
[`permute()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.permute) to specify a
...
The [`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat) function
...
[`F.concat()`](/max/api/python/experimental.functional#max.experimental.functional.concat) since there's
...
- [`sum()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.sum): Sum of elements (tensor method)
- [`mean()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.mean): Average of elements (tensor method)
- [`max()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.max): Maximum value (tensor method)
- [`F.min()`](/max/api/python/experimental.functional#max.experimental.functional.min): Minimum value (functional API only)
...
[`F.sum(x)`](/max/api/python/experimental.functional#max.experimental.functional.sum). Use these when
...
Matrix multiplication is also available as [`F.matmul(x, w)`](/max/api/python/experimental.functional#max.experimental.functional.matmul).
...
[`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu) sets negative
values to zero, [`F.sigmoid()`](/max/api/python/experimental.functional#max.experimental.functional.sigmoid)
maps values to (0, 1), and [`F.tanh()`](/max/api/python/experimental.functional#max.experimental.functional.tanh)
maps values to (-1, 1):
...
The [`max.experimental.random`](/max/api/python/experimental.random) module provides functions for
...
[`random.normal()`](/max/api/python/generated/max.experimental.random.normal) generates values
...
[`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu) from the
...
- **[Building graphs](/max/develop/get-started-with-max-graph-in-python)**: Use operations
