...
- **Core modules**: [`Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module), [`Linear`](/max/api/python/nn/linear#max.nn.linear.Linear), [`Embedding`](/max/api/python/experimental/nn/Embedding#max.experimental.nn.Embedding), [`Sequential`](/max/api/python/experimental/nn/sequential#max.experimental.nn.sequential.Sequential), [`ModuleList`](/max/api/python/experimental/nn/sequential#max.experimental.nn.sequential.ModuleList).
- **Normalization**: [`RMSNorm`](/max/api/python/experimental/nn/norm/RMSNorm#max.experimental.nn.norm.RMSNorm), [`GemmaRMSNorm`](/max/api/python/experimental/nn/norm/GemmaRMSNorm#max.experimental.nn.norm.GemmaRMSNorm).
- **Positional encodings**: [`RotaryEmbedding`](/max/api/python/experimental/nn/rope/RotaryEmbedding#max.experimental.nn.rope.RotaryEmbedding), [`TransposedRotaryEmbedding`](/max/api/python/experimental/nn/rope/TransposedRotaryEmbedding#max.experimental.nn.rope.TransposedRotaryEmbedding).
- **Utilities**: [`module_dataclass`](/max/api/python/experimental/nn/module#max.experimental.nn.module.module_dataclass) for creating module dataclasses.
...
The [`Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module) base class standardizes how
...
[`PagedKVCacheManager`](/max/api/python/kv_cache/paged_kv_cache/cache_manager#max.kv_cache.paged_kv_cache.cache_manager.PagedKVCacheManager),
...
architecture using the [`Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module) class.
...
1. **Define your model**: You create a
   [`Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module) that defines your model
   architecture. The module's
   [`forward`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module.forward) method defines what
...
3. **Pipelines orchestrate execution**: The pipeline receives pre-tokenized
   ...
   model's
   [`PipelineModel.execute()`](/max/api/python/pipelines/interfaces#max.pipelines.lib.interfaces.PipelineModel.execute)
