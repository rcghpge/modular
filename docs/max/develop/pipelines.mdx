...
- [`PipelineModel`](/max/api/python/generated/max.pipelines.PipelineModel):
  ...
  `execute()`, `calculate_max_seq_len()`, and input preparation methods that all
  architectures must implement.
...
and
[`TextGenerationOutput`](/max/api/python/generated/max.pipelines.modeling.types.TextGenerationOutput)
...
pass on the compiled executable. `prepare_initial_token_inputs` builds the
tensors for each scheduled batch (prefill and decode steps share this path in
