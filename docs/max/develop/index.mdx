...
When you run this code, [`Tensor.constant()`](/max/api/python/tensor#max.experimental.tensor.Tensor.constant) creates a tensor and [`F.relu()`](/max/api/python/functional#max.experimental.functional.relu)
...
The [`max.experimental.functional`](/max/api/python/functional) module (typically imported as `F`) provides
operations like [`relu()`](/max/api/python/functional#max.experimental.functional.relu), [`softmax()`](/max/api/python/functional#max.experimental.functional.softmax), [`sqrt()`](/max/api/python/functional#max.experimental.functional.sqrt), and many more.
...
- **Accessing scalar values**: [`x.item()`](/max/api/python/tensor#max.experimental.tensor.Tensor.item) must return a concrete Python number
