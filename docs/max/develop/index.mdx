...
When you run this code, [`Tensor(data)`](/max/api/python/experimental/tensor#max.experimental.tensor.Tensor) creates a tensor and [`F.relu()`](/max/api/python/experimental/functional#max.experimental.functional.relu)
...
- [`max.experimental.nn.Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module):
  uses a `forward()` method and [`compile()`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module.compile)
  to build the same MAX graph automatically. You can't use it directly inside
  a `with Graph(...)` block.
...
The [`max.experimental.functional`](/max/api/python/experimental/functional) module (typically imported as `F`) provides
operations like [`relu()`](/max/api/python/experimental/functional#max.experimental.functional.relu), [`softmax()`](/max/api/python/experimental/functional#max.experimental.functional.softmax), [`sqrt()`](/max/api/python/experimental/functional#max.experimental.functional.sqrt), and many more.
...
- **Accessing scalar values**: [`x.item()`](/max/api/python/experimental/tensor#max.experimental.tensor.Tensor.item) must return a concrete Python number
...
- **Module forward passes**: Calling `module(x)` or `module.forward(x)` on an [`nn.Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module) executes the
