...
The fundamentals pages use [`max.experimental`](/max/api/python/experimental),
an eager, PyTorch-like API that's easy to explore interactively. The
`max.experimental` name is temporary: these APIs are being validated and will
move to new namespaces when they graduate.
...
stable APIs that all MAX production architectures use today. For more
information on both programming styles and when to use each, see
