...
When you run this code, [`Tensor(data)`](/max/api/python/generated/max.experimental.tensor.Tensor) creates a tensor and [`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu)
...
- **Accessing scalar values**: [`x.item()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.item) must return a concrete Python number
...
- **Module forward passes**: Calling `module(x)` or `module.forward(x)` on an [`nn.Module`](/max/api/python/generated/max.experimental.nn.Module) executes the
