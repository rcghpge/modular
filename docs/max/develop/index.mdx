...
When you run this code, [`Tensor.constant()`](/max/api/python/experimental/tensor#max.experimental.tensor.Tensor.constant) creates a tensor and [`F.relu()`](/max/api/python/experimental/functional#max.experimental.functional.relu)
...
- **Module forward passes**: Calling `module(x)` or `module.forward(x)` on an [`nn.Module`](/max/api/python/nn/module_v3/module#max.nn.module_v3.module.Module) executes the
