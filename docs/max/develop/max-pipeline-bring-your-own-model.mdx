...
- **Automated conversion via Hugging Face space**: We can use the [gguf-my-repo](https://huggingface.co/spaces/ggml-org/gguf-my-repo) space
  ...
  You can see all the supported quantization encodings in the [`QuantizationEncoding` module](/max/api/python/generated/max.graph.quantization.QuantizationEncoding).
