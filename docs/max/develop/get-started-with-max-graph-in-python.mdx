...
If you have any questions along the way, ask them on
[our Discord channel](https://discord.gg/modular).
...
[`Graph()`](/max/api/python/generated/max.graph.Graph) class from the MAX Python
API. When initializing, specify a name for the graph and define the types of
...
[`inputs`](/max/api/python/generated/max.graph.Graph#max.graph.Graph.inputs)
property. This returns a symbolic tensor representing the input arguments.
...
[`output()`](/max/api/python/generated/max.graph.Graph#max.graph.Graph.output)
method. This specifies which tensors should be returned when the graph is
executed.
...
that match our graph's input requirements. Then pass the inputs to the
[`execute()`](/max/api/python/generated/max.engine.Model#max.engine.Model.execute)
function:
...
assert that the type is [`Buffer`](/max/api/python/generated/max.driver.Buffer)
so we can convert it to a NumPy array.
