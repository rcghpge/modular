...
[`Tensor`](/max/api/python/tensor#max.tensor.Tensor) like
...
You can create a tensor from a Python list using the [`Tensor.constant()`](/max/api/python/tensor#max.tensor.Tensor.constant) function:
...
This imports the [`Tensor`](/max/api/python/tensor#max.tensor.Tensor) class and
...
such as [`ones()`](/max/api/python/tensor#max.tensor.Tensor.ones)
and [`zeros()`](/max/api/python/tensor#max.tensor.Tensor.zeros):
...
such as [`random.normal()`](/max/api/python/random#max.random.normal):
...
In this example, the [`shape`](/max/api/python/tensor#max.tensor.Tensor.shape)
...
The [`num_elements()`](/max/api/python/tensor#max.tensor.Tensor.num_elements) function returns the total number of elements in the
