...
MAX supports two programming patterns for constructing an AI model:

- *Eager-style execution* for an enhanced developer experience during model
  development and debugging.
- *Explicit graph construction* when you need low-level control over compilation
  and deployment.
...
[`Tensor(data)`](/max/api/python/generated/max.experimental.tensor.Tensor)
creates a tensor and
[`F.relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu)
performs the ReLU activation. The `print(y)` statement triggers execution. MAX
compiles and runs the operations, then displays the result.
...
MAX provides two module APIs, one for each programming pattern:

- [`max.nn.Module`](/max/api/python/nn/layer#max.nn.layer.Module): works
  directly inside `with Graph(...)` and is the right choice for existing
  pipeline code.
- [`max.experimental.nn.Module`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module):
  uses a `forward()` method and
  [`compile()`](/max/api/python/experimental/nn/module#max.experimental.nn.module.Module.compile)
  to build the same MAX graph automatically. You can't use it directly inside a
  `with Graph(...)` block.
...
The [`max.experimental.functional`](/max/api/python/experimental.functional)
module (typically imported as `F`) provides operations like
[`relu()`](/max/api/python/experimental.functional#max.experimental.functional.relu),
[`softmax()`](/max/api/python/experimental.functional#max.experimental.functional.softmax),
[`sqrt()`](/max/api/python/experimental.functional#max.experimental.functional.sqrt),
and many more.
...
- **Accessing scalar values**:
  [`x.item()`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.item)
  must return a concrete Python number (like `3.14`) rather than a symbolic
  operation, which triggers execution.
...
- **Module forward passes**: Calling `module(x)` or `module.forward(x)` on an
  [`nn.Module`](/max/api/python/generated/max.experimental.nn.Module) executes
  the graph to produce the output.
