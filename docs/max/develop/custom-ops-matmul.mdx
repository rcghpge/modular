...
[`TileTensor`](/mojo/std/layout/tile_tensor/TileTensor) to represent the
...
algorithms. For more information, see the [Introduction to
layouts](/mojo/manual/layout/layouts) and [Using
`LayoutTensor`](/mojo/manual/layout/tensors) sections of the Mojo Manual.
...
The
[`GenericToSharedAsyncTileCopier`](/mojo/std/layout/tile_io/GenericToSharedAsyncTileCopier)
...
the
[`TileTensor.vectorize()`](/mojo/std/layout/tile_tensor/TileTensor#vectorize)
...
row-major shared memory, but `a_smem` switches to column-major so the inner
loop can read `TM` contiguous M-elements at a fixed K. Each thread does a
vectorized K-load from global `A` and scalar-scatters the lanes into `a_smem`,
transposing `A` on the way in.
