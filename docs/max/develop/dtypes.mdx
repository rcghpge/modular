...
In this example, the [`ones()`](/max/api/python/tensor#max.experimental.tensor.Tensor.ones) function creates a tensor filled with ones, and
the [`constant()`](/max/api/python/tensor#max.experimental.tensor.Tensor.constant) function creates a tensor filled with the given values. The
...
Every tensor has a [`dtype`](/max/api/python/tensor#max.experimental.tensor.Tensor.dtype) property that returns its data type:
...
The [`cast()`](/max/api/python/tensor#max.experimental.tensor.Tensor.cast) method is used to convert a tensor from one dtype to another. This
...
In this example, the [`from_dlpack()`](/max/api/python/tensor#max.experimental.tensor.Tensor.from_dlpack) method converts the NumPy array to a MAX tensor.
