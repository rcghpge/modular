...
graph compiler: the parameters, inputs, and outputs. The `execute()` function must
include `target` as the first compile-time parameter and `output` as the first
runtime argument; the graph compiler uses this order to identify the op's hardware
target and output slots.
...
The [`Graph()`](/max/api/python/generated/max.graph.Graph) takes an input tensor with five
...
Notice that the [`Buffer`](/max/api/python/generated/max.driver.Buffer) is initially
