...
| `MAX_EAGER_OP_PRECOMPILE`   | Controls how the eager interpreter compiles its built-in op targets. The default precompiles the full (device, dtype) matrix at import so steady-state dispatch never recompiles. Set to `0` to skip the import-time sweep and compile each target lazily on its first dispatch, which avoids a long cold-cache compile when a program touches only a few targets. MAX reads the value when the sweep runs, not at import. | `0`, `1`        | `1`                             |
