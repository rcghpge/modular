...
files), see [Image and video to text](/max/inference/image-to-text).

:::

Both the
[`/chat/completions`](/max/rest-api/#POST/v1/chat/completions) and
[`/completions`](/max/rest-api/#POST/v1/completions) API endpoints are
...
:::note Schema root type

The root of a `json_schema` must resolve to an object. MAX infers `type: object`
when the root omits a `type`, and accepts a `type` union that includes `object`
(for example, `["object", "array", "string"]`). A root pinned to a single
non-object type, such as `{"type": "string"}`, is rejected.
