...
compile the function first and pass it to `ctx.enqueue_function` in this
scenario:
...
programmers, showing how to define and dispatch GPU functions within a single
Mojo file. These examples recreate the first three samples from the popular
textbook
...
Mandelbrot set calculation example that parallels a similar one in the existing
custom ops examples.
