...
- Added a [video generation](/max/inference/video-generation/) guide for serving
  text-to-video models with the `v1/responses` endpoint.
- Added new model developer guides covering
  [reading a Hugging Face model config](/max/develop/read-model-config/),
  [bringing up models with AI agent skills](/max/develop/model-bringup-skills/),
  and [tensor realization](/max/develop/tensor-realization/).
- Expanded [AI coding assistants](/max/coding-assistants/) documentation with
  Modular skills for automating new model bring-up with the `import-model`
  skill.
- Updated guides for
