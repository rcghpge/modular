...
The optimized attention layers include variants that accept an attention mask.
More memory-efficient variants that don't take a mask instead take a "mask
functor" argument to the kernel, which implements masking without materializing
a mask by computing a mask value from input coordinates on the fly.
...
These include an initial implementation of ragged tensors—tensors for which each
dimension can have a different size, avoiding the use of padding tokens by
flattening a batch of sequences of differing lengths.
...
These are custom fused kernels that update the KV cache in place. Although they
are custom, they reuse the underlying kernel implementation by passing in lambda
functions used to retrieve inputs and write to outputs in place.
...
features like graph-based multi-step scheduling, sampling, KV cache management,
ragged tensor support, and more.
