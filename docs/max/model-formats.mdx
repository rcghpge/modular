...
1. Instantiate a [`Graph`](/max/api/python/generated/max.graph.Graph) by specifying the input
shape as a
[`TensorType`](/max/api/python/generated/max.graph.TensorType).

2. Build the graph by chaining [`ops`](/max/api/python/graph.ops) functions.
Each function takes and returns a [`Value`](/max/api/python/generated/max.graph.Value)
object.

3. Add the final `Value` to the graph using the
[`output()`](/max/api/python/generated/max.graph.Graph#max.graph.Graph.output) method.
